...
class SelfForcingFlowEuler:
    ...
    def _initialize_kv_cache(self, num_chunks: int) -> list[list[list[torch.Tensor | None]]]:
        kv_cache: list[list[list[torch.Tensor | None]]] = []
        slots_per_block = 6 if self.use_softmax_attention else 3
        for _ in range(num_chunks):
            kv_cache.append([[None] * slots_per_block for _ in range(self.num_model_blocks)])
        return kv_cache

    def _accumulate_softmax_kv_cache(self, kv_cache: list, chunk_idx: int) -> tuple[list, int, bool]:
        """Accumulate softmax KV cache (``[q, k, v, beta, decay, x_qk]``)."""
        if chunk_idx == 0:
            return kv_cache[0], 0, self.total_frames % self.base_chunk_frames

        cur_kv_cache = kv_cache[chunk_idx]
        for block_id in range(self.num_model_blocks):
            previous_q = previous_k = previous_v = None
            previous_beta = previous_decay = previous_x_qk = None
            start_chunk_idx = chunk_idx - self.num_cached_blocks if self.num_cached_blocks > 0 else 0
            for i in range(chunk_idx):
                if i < start_chunk_idx:
                    kv_cache[i][block_id] = [None, None, None, None, None, None]
                    continue
                ...
                # prev[5] may be conv cache, not aligned with prev[3].
                if prev[5] is not None:
                    if previous_x_qk is None:
                        previous_x_qk = prev[5].clone()
                    else:
                        previous_x_qk = torch.cat([previous_x_qk, prev[5]], dim=2)

                if prev[3] is not None:
                    if previous_beta is None:
                        previous_beta = prev[3].clone()
                        previous_decay = prev[4].clone()
                    else:
                        previous_beta = torch.cat([previous_beta, prev[3]], dim=1)
                        previous_decay = torch.cat([previous_decay, prev[4]], dim=-1)
            if chunk_idx > 0:
                assert previous_q is not None and previous_k is not None
        ...
        is_sink_token = (self.total_frames % self.base_chunk_frames == 1 and start_chunk_idx == 0) or self.sink_token
        return cur_kv_cache, chunk_idx - start_chunk_idx, is_sink_token

    def _accumulate_linear_kv_cache(self, kv_cache: list, chunk_idx: int) -> tuple[list, int, bool]:
        """Accumulate linear-attention KV cache (``[cum_vk, cum_k_sum, tconv]``)."""
        if chunk_idx == 0:
            return kv_cache[0]

        cur_kv_cache = kv_cache[chunk_idx]
        for block_id in range(self.num_model_blocks):
            # Carry tconv state forward from the previous chunk.
            cur_kv_cache[block_id][2] = kv_cache[chunk_idx - 1][block_id][2]
            cum_vk: torch.Tensor | None = None
            cum_k_sum: torch.Tensor | None = None

            start_idx = max(chunk_idx - self.num_cached_blocks, 0) if self.num_cached_blocks > 0 else 0
        ...
        return cur_kv_cache, chunk_idx - start_idx, False
    ...
    def accumulate_kv_cache(self, kv_cache: list, chunk_idx: int):
        ...
        return self._accumulate_linear_kv_cache(kv_cache, chunk_idx)
    ...
    def sample(
        ...
        **kwargs: object,
        ...
        """Autoregressively denoise ``latents`` chunk by chunk."""
        ...
        assert self.condition.shape[0] == batch_size or self.condition.shape[0] == num_chunks, (
            f"condition shape: {self.condition.shape}, batch_size: {batch_size}, " f"num_chunks: {num_chunks}"
        )
        ...
        for chunk_idx in range(num_chunks):
            chunk_kv_cache, num_chunks_to_accumulate, is_sink_token = self.accumulate_kv_cache(kv_cache, chunk_idx)
            ...
            for i, t in tqdm(
                ...
                noise_pred, _ = self.model(
                    ...
                    timestep,
                    prompt_embeds,
                    start_f=chunk_indices[cache_start_chunk_idx],
                    end_f=end_f,
                    save_kv_cache=False,
                ...
                latents[:, :, start_f:end_f] = self.scheduler.step(
                    noise_pred,
                    t,
            ...
            # Extra forward to populate the KV cache for the next chunk.
            latent_model_input = (
                torch.cat([latents[:, :, start_f:end_f]] * 2)
                if do_classifier_free_guidance
                else latents[:, :, start_f:end_f]
        ...
        **kwargs: object,
        ...
        for chunk_idx in range(num_chunks):
            ...
            if _explicit_sigmas is not None:
                # Override timestep schedule with the student's exact training
                # subsampled list. ``shift=1.0`` lets pre-shifted sigmas pass
                # through unchanged.
            ...
            for i, t in tqdm(
                ...
                timestep_tensor = (1.0 - condition_mask_chunk) * timestep_tensor
            ...
            # KV cache save pass — only needed for KV-cache mode.
            latent_model_input = (
                torch.cat([latents[:, :, start_f:end_f]] * 2)
                if do_classifier_free_guidance
                else latents[:, :, start_f:end_f]
