...
- EncoderCacheManager: Manages encoder cache state for streaming inference
"""
...
def _env_flag(name: str, default: bool = False) -> bool:
    """Read boolean flag from environment variable."""
...
def _parse_int_set_env(name: str) -> set[int] | None:
...
class EncoderCacheManager:
    ...
    Similar to DecoderCacheManager but for encoder streaming operations.
    """
    ...
    def check_pending_consumed(self) -> None:
        ...
        Raises:
            RuntimeError: If there are pending frames in downsampler cache
        """
...
class AutoencoderKLCausalLTX2Video(ModelMixin, AutoencoderMixin, ConfigMixin, FromOriginalModelMixin):
    ...
    @property
    def _decoder_feat_map(self) -> list:
        """Legacy property for decoder feature map."""
        return self._decoder_cache.feat_map

    @_decoder_feat_map.setter
    def _decoder_feat_map(self, value: list):
        """Setter for legacy decoder feature map property."""
        self._decoder_cache._state.feat_map = value

    @property
    def _decoder_chunk_is_first(self) -> bool:
        """Legacy property for decoder first chunk flag."""
        return self._decoder_cache.is_first_chunk

    @_decoder_chunk_is_first.setter
    def _decoder_chunk_is_first(self, value: bool):
        """Setter for legacy decoder first chunk flag."""
        self._decoder_cache._state.is_first_chunk = value

    @property
    def _decoder_prev_latent_tail(self) -> torch.Tensor | None:
        """Legacy property for decoder previous latent tail."""
        return self._decoder_cache.prev_latent_tail

    @_decoder_prev_latent_tail.setter
    def _decoder_prev_latent_tail(self, value: torch.Tensor | None):
        """Setter for legacy decoder previous latent tail."""
        self._decoder_cache._state.prev_latent_tail = value

    @property
    def _decoder_cache_mode(self) -> bool | None:
        """Legacy property for decoder cache mode."""
        return self._decoder_cache.cache_mode

    @_decoder_cache_mode.setter
    def _decoder_cache_mode(self, value: bool | None):
        """Setter for legacy decoder cache mode."""
        self._decoder_cache._state.cache_mode = value
    ...
    def encode_with_cache(
        ...
        """Encode video chunks with persistent per-layer causal feature cache.

        Args:
            x: Input video tensor [B, C, T, H, W]
            causal: Whether to use causal encoding
            return_dict: Whether to return dict or tuple
            reset_cache: Whether to reset cache before encoding
        ...
        if self.use_slicing and x.shape[0] > 1:
            raise ValueError("encode_with_cache does not support batch slicing; pass batch size 1 or disable slicing.")
    ...
    def encode(
        ...
        encoded_chunks = []
        start = 0
        step = first_chunk_num_frames
        num_frames = x.shape[2]
        while start < num_frames:
            end = min(start + step, num_frames)
            encoded_chunk = self.encoder(
                x[:, :, start:end, :, :], causal=causal, feat_cache=self._encoder_cache.feat_map, feat_idx=[0]
            )
        ...
        # Validate that all temporal grouping state has been fully consumed
    ...
    def decode(
        ...
        causal: bool | None = None,
        return_dict: bool = True,
        chunk_num_latent_frames: int = 3,
        prepend_prev_latent_frames: int = 3,
        frame_shift: int = 1,
    ) -> DecoderOutput | tuple[torch.Tensor]:
        """Decode full latent video chunk-by-chunk with persistent decoder feature cache.

        This method splits the input latent into chunks and decodes them sequentially,
        using the per-layer feature cache to maintain temporal consistency between chunks.

        Note: prepend_prev_latent_frames and frame_shift are kept for backward compatibility
        but are not used. The chunking relies only on per-layer feature caching.
        ...
        Args:
            ...
            causal: Whether to use causal decoding
            return_dict: Whether to return dict or tuple
            chunk_num_latent_frames: Number of latent frames per chunk
            prepend_prev_latent_frames: Not used (backward compatibility)
            frame_shift: Not used (backward compatibility)

        Returns:
            DecoderOutput containing decoded full video
        """
        if self.use_slicing and z.shape[0] > 1:
            raise ValueError(
                "decode_full_video_with_cache does not support batch slicing; pass batch size 1 or disable slicing."
            )
        if chunk_num_latent_frames <= 0:
            raise ValueError(f"`chunk_num_latent_frames` must be > 0, got {chunk_num_latent_frames}.")
        if z.shape[2] <= 0:
            raise ValueError("Input latent video must contain at least 1 frame.")

        # Kept in the signature for backward compatibility; decode chunking relies only on feature cache.
        _ = prepend_prev_latent_frames, frame_shift

        causal = self.decoder.is_causal if causal is None else causal
        num_latent_frames = z.shape[2]

        # Calculate first chunk size for alignment
        first_chunk_num_latent_frames = num_latent_frames % chunk_num_latent_frames + chunk_num_latent_frames
        ...
        decoded_chunks = []
        start = 0
        step = first_chunk_num_latent_frames if first_chunk_num_latent_frames > 0 else chunk_num_latent_frames

        while start < num_latent_frames:
            end = min(start + step, num_latent_frames)
            z_chunk = z[:, :, start:end, :, :]

            # Decode chunk (rely on feat_cache for temporal consistency)
            feat_idx = [0]
            ...
            start = end
            step = chunk_num_latent_frames

        decoded = decoded_chunks[0] if len(decoded_chunks) == 1 else torch.cat(decoded_chunks, dim=2)
    ...
    # -------------------------------------------------------------------------
    # Standard Encode/Decode Methods
    # -------------------------------------------------------------------------
