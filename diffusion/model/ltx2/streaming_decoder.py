...
class CausalVaeStreamingDecoder:
    ...
    def decode_chunk(self, z_chunk: torch.Tensor) -> torch.Tensor:
        ...
        # decode_with_cache itself never resets the cache between calls; we reset
        # explicitly on the first chunk so successive `decode_chunk` invocations
        # build on the same feat_map.
        reset = self._first_chunk
        result = vae.decode_with_cache(
            z_unnormalized,
            temb=None,
            causal=True,
            return_dict=False,
            reset_cache=reset,
            prepend_prev_latent_frames=0,
        )
