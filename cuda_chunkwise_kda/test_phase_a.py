...
from cuda_impl import cuda_phase_a, build
...
def cmp(name, a, b):
    ...
    print(f"  {name:8s} shape={tuple(a.shape)} max_abs={d.max():.3e} "
...
def main():
    ...
    IPk_t, A_t, IPz_t, Bz_t = phase_a(
        inp["qkv"], inp["beta"], inp["q_inv_rms"], inp["k_inv_rms"], inp["q_norm_w"],
        inp["k_norm_w"], inp["rope_cos"], inp["rope_sin"], F=F, S=S, dot_precision=0)
    ...
    ext = ci.build()
    qkv = inp["qkv"].contiguous(); beta = inp["beta"].float(); kir = inp["k_inv_rms"].float()
    knw = inp["k_norm_w"].float(); rc = inp["rope_cos"]; rs = inp["rope_sin"]
    BH = 20
    BD = 128
    dev = qkv.device
    IPk = torch.empty(BH, F, BD, BD, device=dev, dtype=torch.bfloat16)
    A = torch.empty_like(IPk); IPz = torch.empty_like(IPk)
