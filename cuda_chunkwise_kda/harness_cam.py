...
from diffusion.model.ops.fused_gdn_chunkwise import (
    cam_scan_bidi_chunkwise, phase_a, phase_b_triton, phase_c, _cam_identity_tables)
from harness import time_ms
...
def main():
    ...
    B, H, D, N = q.shape
    F = beta.shape[2]; S = N // F
    qkv = torch.empty(B, N, 3, H, D, device=dev, dtype=q.dtype)
    qkv[:, :, 0].copy_(q.permute(0, 3, 1, 2)); qkv[:, :, 1].copy_(k.permute(0, 3, 1, 2)); qkv[:, :, 2].copy_(v.permute(0, 3, 1, 2))
    oir, onw, ocos, osin = _cam_identity_tables(B=B, N=N, H=H, D=D, device=dev)

    def pa():
        return phase_a(qkv, beta, oir, oir, onw, onw, ocos, osin, F=F, S=S, k_scale=1.0,
                       dot_precision=0, skip_relu=True, skip_z=True)
    Ip, A, Iz, Bz = pa()
    def pb():
        return phase_b_triton(Ip, A, Iz, Bz, decay, F=F, dot_precision=0, direction=0,
                              combined_history=True, skip_z=True)
    Mh, zh, _, _ = pb()
    def pc():
        return phase_c(qkv, oir, onw, ocos, osin, Mh, zh, F=F, S=S, dot_precision=0,
                       skip_relu=True, num_only=True)
    num, _ = pc()
    def transpose():
    ...
    # ── CUDA cam path ──
    from cuda_impl import run_cuda_cam, build
    build()
    ...
    ext = ci.build()
    BH = B * H; BD = 128
    IPk = torch.empty(BH, F, BD, BD, device=dev, dtype=torch.bfloat16); A_ = torch.empty_like(IPk)
    betaf = beta.contiguous().float(); kc = k.contiguous(); vc = v.contiguous(); qc = q.contiguous()
    ...
    Mh, _, _, _ = phase_b_triton(IPk, A_, dummy, dummy, decay.contiguous(), F=F, dot_precision=0, direction=0, combined_history=True, skip_z=True)
    tcb = time_ms(lambda: phase_b_triton(IPk, A_, dummy, dummy, decay.contiguous(), F=F, dot_precision=0, direction=0, combined_history=True, skip_z=True), args.warmup, args.iters)
    Mbf = Mh.to(torch.bfloat16).contiguous(); oo = torch.empty(B, H, D, N, device=dev, dtype=torch.float32)
