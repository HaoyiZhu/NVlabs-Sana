...
def _build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--stage1_precision", choices=["bf16", "fp8", "fp4"], default="bf16",
                   help="Stage-1 DiT compute precision. bf16 (default, any GPU); "
                        "fp8 = FP8 W8A8 (Hopper+ / Blackwell); fp4 = NVFP4 W4A4 (Blackwell only). "
                        "Quantizes self-attn + cross-attn + FFN, per-block scoped.")
    p.add_argument("--refiner_precision", choices=["bf16", "fp8", "fp4"], default="bf16",
    ...
    if "fp4" in (args.stage1_precision, args.refiner_precision) and torch.cuda.is_available():
        if torch.cuda.get_device_capability()[0] < 10:  # NVFP4 needs Blackwell (sm_100/sm_120)
            logger.warning(
                ...
                "Use fp8 on Hopper.", *torch.cuda.get_device_capability()
            )
...
def main() -> None:
    ...
    for run_idx in range(args.benchmark_repeats):
        ...
        logger.info(
            ...
            f"realtime={result['realtime_factor']:.3f}x "
    ...
    if args.benchmark_json is not None:
        ...
        payload = {
            ...
            "peak_alloc_gb": max(run["peak_alloc_gb"] for run in run_payloads),
            "peak_reserved_gb": max(run["peak_reserved_gb"] for run in run_payloads),
        }
